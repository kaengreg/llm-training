...
class LoraTrainingArguments(TrainingArguments):
    ...
    rank_pattern_path: Optional[str] = field(default=None)
    alpha_pattern_path: Optional[str] = field(default=None)
...
def main():
    ...
    logger.warning(int(os.environ["LOCAL_RANK"]) if 'LOCAL_RANK' in os.environ else '')
    ...
    print(os.environ.get('RUADAPT_NO_TRAIN', False))
    ...
    if int(os.environ.get('RUADAPT_NO_TRAIN', 0)) == 1:
        return 0
    ...
    # Freeze all layers but input and output embeddings
    if training_args.peft:
        ...
        logger.info(f"use dora: {use_dora}")
        logger.info(f"target_modules: {target_modules}")
        logger.info(f"default lora_rank: {lora_rank}")
        rank_pattern = {}
        alpha_pattern = {}
        if training_args.rank_pattern_path is not None:
